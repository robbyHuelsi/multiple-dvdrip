...
class DVD:
    ...
    def ScanTitles(self, title_numbers, verbose):
        ...
        to_scan = [x for x in range(1, title_count + 1) if x != first and ((not title_numbers) or x in title_numbers)]
        for i in to_scan:
            try:
        ...
        if os.name == "nt":
            if len(self.mountpoint) < 4 and self.mountpoint[1] == ":":
                ...
                ctypes.windll.WINMM.mciSendStringW(
                    f"open {drive_letter}: type CDAudio alias {drive_letter}_drive", None, 0, None
                )
...
def FindMainFeature(titles, verbose=False) -> None:
    ...
    main_feature = max(titles, key=lambda title: ParseDuration(title.info["duration"]))
    if verbose:
...
DURATION_REGEX = re.compile(r"^(?:.*,)?\s*duration\s+(\d\d):(\d\d):(\d\d)\s*(?:,.*)?$")
...
SubtitleTrack = namedtuple("SubtitleTrack", "number info")
...
def DisplayScan(titles) -> None:
    ...
    for title in titles:
        ...
        print(
            ...
            % (title.number, len(titles), duration, size.width, size.height, xaspect, yaspect, size.fps)
        )
        for at in ParseAudioTracks(info["audio tracks"]):
            print("  audio % 3d: %s (%sch)  [%s]" % (at.number, at.lang, at.channels, at.extras))
        for sub in ParseSubtitleTracks(info["subtitle tracks"]):
            print("  sub % 3d: %s" % (sub.number, sub.info))
        position = 0
        if title_seconds > 0:
...
def ParseArgs():
    ...
    parser.add_argument(
        "-c", "--chapter_split", action="store_true", help="Split each chapter out into a separate file."
    )
    parser.add_argument("-n", "--dry-run", action="store_true", help="Don't actually write anything.")
    parser.add_argument("--scan", action="store_true", help="Display scan of disc; do not rip.")
    parser.add_argument("--main-feature", action="store_true", help="Rip only the main feature title.")
    parser.add_argument(
        "-t",
        "--titles",
        default="*",
    ...
    parser.add_argument("-i", "--input", help="Volume to rip (must be a directory).", required=True)
    parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument(
        "--mount-timeout", default=15, help="Amount of time to wait for a mountpoint to be mounted", type=float
    )
...
def main() -> None:
    ...
    if args.scan:
        ...
        PerformTasks(dvd, tasks, len(titles), filenames, dry_run=args.dry_run, verbose=args.verbose)
